...
class ConnectHandshakeTest : public ::testing::Test {
 protected:
  ...
  ~ConnectHandshakeTest() { io_context_.stop(); }
...
TEST_F(ConnectHandshakeTest, Run) {
  ConnectClient();
  ...
  auto main = [&]() -> asio::awaitable<void> {
    ...
    ConnectHandshake handshake{client_socket_, target_server_addr,
                               auth_options};
